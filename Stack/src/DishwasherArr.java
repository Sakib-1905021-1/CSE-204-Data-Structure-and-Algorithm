...
public class DishwasherArr {
    ...
    public static void main(String[] args) {
        ...
        n = input.nextInt();
        x = input.nextInt();
        Stack<Integer> washTime = new ArrStack<>(x);
        for(int i=0; i<x; i++){
            washTime.push(input.nextInt());
        }
        Stack<Integer>friendId = new ArrStack<>(n*x);
        Stack<Integer>dirtyStack = new ArrStack<>(n*x);
        Stack<Integer>courseNum = new ArrStack<>(n*x);
        for(int i=0; i<(n*x); i++){
            k = input.nextInt();
            t = input.nextInt();
            s = input.nextInt();
